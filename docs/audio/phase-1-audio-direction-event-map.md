...
| condenser_water_produced | Hear that one approved production cycle created Clean Water | Brief contained condensation/liquid-ready accent | DES-004 1 Clean Water per 90 active powered seconds | A valid RUNNING production cycle completes and output count increases by one | NO | STANDARD | WORLD_3D | Sits above running loop; does not stop it unless output reaches full | Small restrained variation | Do not imply player collected water, personal XP, extra output, or offline production | ONE_SHOT; SCOPE WORLD_ENTITY |
...
| survival_critical_dehydration | Warn that Water reached 0 and approved Health drain is active | Highest-urgency dehydration state-entry warning, distinct from individual hurt ticks | DES-003 Water = 0 | Local player enters CRITICAL DEHYDRATION | NO | CRITICAL | PLAYER_LOCAL/UI_LOCAL | Ducks beds/music briefly; player_hurt remains the damage-tick confirmation | Stable state-entry identity | Do not invent extra damage rate; approved Health loss is 1 every 5 s | ONE_SHOT; SCOPE LOCAL_PLAYER |
...
| survival_critical_starvation | Warn that Food reached 0 and approved Health drain is active | Highest-urgency starvation state-entry warning, separate from hurt ticks | DES-003 Food = 0 | Local player enters CRITICAL STARVATION | NO | CRITICAL | PLAYER_LOCAL/UI_LOCAL | Ducks beds/music briefly; player_hurt confirms each actual damage event | Stable state-entry identity | Do not invent extra damage rate; approved Health loss is 1 every 10 s | ONE_SHOT; SCOPE LOCAL_PLAYER |
...
| survival_critical_cold | Warn that thermal state reached the approved highest cold severity | Extreme cold state-entry warning with clear continuity from severe cold | DES-003 CRITICAL COLD 0–4 | Local player enters CRITICAL COLD | NO | CRITICAL | PLAYER_LOCAL/UI_LOCAL | Highest survival priority; player_hurt confirms actual 1-per-3-s damage ticks | Stable state-entry identity | Do not imply instant death, frostbite subsystem, or weather-direct damage | ONE_SHOT; SCOPE LOCAL_PLAYER |
